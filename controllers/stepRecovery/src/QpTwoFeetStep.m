...
block.InputPort(2).Dimensions        = [18 18];   % HessianMatrixQP2Feet               
block.InputPort(3).Dimensions        = [ 1 18];   % gradientQP2Feet
block.InputPort(4).Dimensions        = [38 18];   % ConstraintsMatrixQP2Feet 
...
block.OutputPort(4).Dimensions       = 6;        % slack variables  
...
function Outputs(block)
    ...
    if sum(LEFT_RIGHT_FOOT_IN_CONTACT) > (2 - CONTACT_THRESHOLD)
        ...
        if USE_QPO_SOLVER 
            [f02Feet_slack,~,exitFlagQP,~,~,~] = qpOASES(HessianMatrixQP2Feet,gradientQP2Feet',ConstraintsMatrixQP2Feet,[],[],[],bVectorConstraintsQp2Feet');           
            f02Feet = f02Feet_slack(1:12);
            slack = f02Feet_slack(13:end);
            if exitFlagQP ~= 0
                f02Feet = - inv(HessianMatrixQP2Feet(1:12,1:12))*gradientQP2Feet(:,1:12)';
            end
        else
            exitFlagQP                 = 1;
            f02Feet = - inv(HessianMatrixQP2Feet(1:12,1:12))*gradientQP2Feet(:,1:12)';
