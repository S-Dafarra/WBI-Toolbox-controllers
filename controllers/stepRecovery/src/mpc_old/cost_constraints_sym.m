function [hessian,gradient,C,B,Ceq,Beq,Cleq,Bleq,fRH,time] = cost_constraints_sym(m, Cl, Bl, Cr, Br, Cch, Bch, Alr, omega, g, ref,gains, gamma0, nsteps, T, k_impact)
%Inputs
...
%Ev = zeros(state_dim);
Ev = sym(zeros(9*nsteps,state_dim));
...
C_horL = sym(zeros(nsteps*ncl,state_dim)); %initialization of constraints matrix on the left foot over the horizon

ncr = size(Cr,1);  %number of constraints on the right foot
if (k_impact > nsteps)
    C_horR = sym(zeros(nsteps*6,state_dim)); %initialization for the constraint matrix on the right foot for the time instants before the impact 
    C_horRI = sym(zeros(0,state_dim));     %just a null matrix, but it has to be defined
else if k_impact>0
        C_horR = sym(zeros(6*(k_impact-1),state_dim));
        C_horRI = sym(zeros(ncr*(nsteps-k_impact+1),state_dim)); %initialization for the constraint matrix on the right foot for the time instants AFTER the impact 
    else C_horR = sym(zeros(0,state_dim));
         C_horRI = sym(zeros(nsteps*ncr,state_dim));
    end
end

 %if Cch*x<=Bch then x is in the convex hull
nch = size(Cch,1);
if (k_impact > nsteps)
    C_horCH = sym(zeros(0,state_dim));
else if k_impact>0
        C_horCH = sym(zeros(nch*(nsteps-k_impact+1),state_dim));
    else C_horCH = sym(zeros(nsteps*nch,state_dim));
    end
end

C_horZ = sym(zeros(nsteps,state_dim));
...
for i = 1:nsteps
   %% Constraints introduced by the dynamics
    ...
    C_horZ(i,:) = [0,0,-1,zeros(1,6)]*[zeros(9,9*(i-1)),eye(9),zeros(9,9*(nsteps-i)),zeros(9,12*nsteps)];
...
K_hor_gammaTer = diag([zeros(9*(nsteps-1),1);reshape(gains.TerCOM,[],1)]);
